...
export function getIztroChartData(
  ...
  try {
    ...
    const rawPalaces = chart.palaces as any[]

    // --- Mệnh / Thân cung ---
    // iztro palace index starts from Dần; convert: ourIndex = (iztroIndex + 2) % 12
    const menhPalace = rawPalaces.find((p) => (p.name as string) === 'Mệnh')
    const thanPalace = rawPalaces.find((p) => p.isBodyPalace)
    const cungMenhIndex = menhPalace ? (menhPalace.index + 2) % 12 : 0
    const cungThanIndex = thanPalace ? (thanPalace.index + 2) % 12 : 0

    // --- 12 Cung ---
    const palaces: Palace[] = rawPalaces.map((p) => {
      const ourIndex = (p.index + 2) % 12

      // 14 Chính tinh
      const mainStars: Star[] = (p.majorStars as any[]).map((s) => ({
        name: s.name,
    ...
    const daiHan: DaiHan[] = rawPalaces
      .map((p) => {
        const ourIndex = (p.index + 2) % 12
