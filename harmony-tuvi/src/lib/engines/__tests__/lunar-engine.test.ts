...
describe('Lunar Engine - Solar Terms (Tiết Khí)', () => {
  describe('getTietKhi', () => {
    it('should identify solar terms correctly near their start dates', () => {
      const tietKhi = getTietKhi(new Date(2024, 2, 20)) // Xuân phân approx 20-3
      expect(tietKhi).toBeDefined()
    })

    it('should return undefined for dates far from any solar term start', () => {
      const tietKhi = getTietKhi(new Date(2024, 2, 10))
      expect(tietKhi).toBeUndefined()
    })
  })
})
